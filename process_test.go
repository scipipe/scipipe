...
func TestSetPathStatic(t *testing.T) {
	...
	mockTask := NewTask(wf, p, "echo_foo_task", "", nil, nil, nil, nil, "", p.ExecMode, 1)
...
func TestSetPathExtend(t *testing.T) {
	...
	mockTask := NewTask(wf, p, "echo_foo_task", "", map[string]*FileIP{"foo": NewFileIP("foo.txt")}, nil, nil, nil, "", p.ExecMode, 1)
